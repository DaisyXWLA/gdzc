      ...
      <Row>
        <Cell :width="24">
          ...
          >
            ...
            <FormItem label="存放地点" prop>
              <input type="text" v-model="BookAdd.storageLocation" v-wordlimit="50"/>
            </FormItem>
        ...
        <Cell :width="24" align="center">
          <div class="submit-btn">
            <Button color="blue" @click="submit">更新</Button>
          </div>
        </Cell>
      </Row>
    </div>
  ...
  methods: {
    ...
    submit() {
      ...
      if (validResult.result) {
        ...
        R.Assets.booksFileModify(BookModel.dispose(this.BookAdd)).then(resp => {
          if (resp.ok) {
            setTimeout(() => {
              this.loading = false;

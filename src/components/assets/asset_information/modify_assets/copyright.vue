  ...
  methods: {
    ...
    getAssetsId(id) {
      ...
      R.Assets.findCopyright(copyrightModel.dispose(this.copyrightAdd)).then(resp => {
        if (resp.ok) {
          this.loading = false;
          this.copyrightAdd = copyrightModel.parse(resp.body);
        } else {
          this.loading = false;
          this.$Message('获取数据失败');
        }
    ...
    submit() {
      ...
      if (this.validResult.result) {
        ...
        R.Assets.copyrightModify(copyrightModel.dispose(this.copyrightAdd)).then(resp => {
          if (resp.ok) {
            setTimeout(() => {
              this.loading = false;
    ...
    // 上传附件
    onUpload: function (e) {
      this.copyrightAdd.file = e.target.files[0];
      R.Upload.Upload(copyrightModel.dispose(this.copyrightAdd)).then(resp => {
        if (resp.ok) {
        }
      });
    },

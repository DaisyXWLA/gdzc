...
export default {
  ...
  mounted() {
    this.GoodWillAdd.fiscalFunds = 0;
    this.GoodWillAdd.noFiscalFunds = 0;
  },
  methods: {
    getcategoryName(data) {
      this.GoodWillAdd.assetsCategoryName = data.categoryName;
      this.GoodWillAdd.assetsCategory = data.code;
      this.getList();
      this.loading = false;
    },
    getList() {
    ...
    submit() {
      ...
      if (validResult.result) {
        ...
        R.Assets.goodwillAdd(GoodWillModel.dispose(this.GoodWillAdd)).then(resp => {
          if (resp.ok) {
            this.loading = false;
            this.$Message.success('保存成功');
            this.assetsClassName = this.GoodWillAdd.assetsCategoryName;
            this.GoodWillAdd = GoodWillModel.parse({});
            this.GoodWillAdd.assetsCategoryName = this.assetsClassName;
          } else {
            this.loading = false;

...
export default {
  ...
  mounted() {
    this.brandAdd.fiscalFunds = 0;
    this.brandAdd.noFiscalFunds = 0;
  },
  methods: {
    getcategoryName(data) {
      this.assetsCategoryName = data.categoryName;
      this.brandAdd.assetsCategoryName = data.categoryName;
      this.brandAdd.assetsCategory = data.code;
      this.getList();
      this.loading = false;
    },
    // 获取部门数据
    ...
    submit() {
      ...
      if (validResult.result) {
        ...
        R.Assets.brandAdd(brandModel.dispose(this.brandAdd)).then(resp => {
          if (resp.ok) {
            this.loading = false;
            this.$Message.success('保存成功');
            this.brandAdd = brandModel.parse({});
            this.brandAdd.assetsCategoryName = this.assetsCategoryName;
          } else {
            this.loading = false;

        ...
        <Cell :width="24">
          ...
          >
            ...
            <FormItem label="数量" prop="number">
              <NumberInput v-model="DisplayAdd.number"></NumberInput>
            </FormItem>
            ...
            <FormItem label="使用部门">
              <Select
                v-model="DisplayAdd.useDepartment"
                class="select-demo"
                :datas="useDepartment"
                :filterable="true"
              ></Select>
...
export default {
  ...
  mounted() {
    this.DisplayAdd.fiscalFunds = 0;
    this.DisplayAdd.noFiscalFunds = 0;
  },
  methods: {
    getcategoryName(data) {
      this.assetsCategoryName = data.categoryName;
      this.DisplayAdd.assetsCategoryName = data.categoryName;
      this.DisplayAdd.assetsCategory = data.code;
      this.getList();
      this.loading = false;
    },
    getList() {
    ...
    submit() {
      ...
      if (validResult.result) {
        ...
        R.Assets.culturalRelicAdd(DisplayModel.dispose(this.DisplayAdd)).then(resp => {
          if (resp.ok) {
            this.loading = false;
            this.$Message.success('保存成功');
            this.DisplayAdd = DisplayModel.parse({});
            this.DisplayAdd.assetsCategoryName = this.assetsCategoryName;
          } else {
            this.loading = false;

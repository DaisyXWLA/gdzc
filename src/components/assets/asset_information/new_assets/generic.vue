      ...
      <Row>
        ...
        <Cell :width="24" align="center">
          <div class="submit-btn">
            <Button color="blue" size="l" @click="submit">提交</Button>
          </div>
        </Cell>
      </Row>
    </div>
...
export default {
  ...
  mounted() {
    this.genericAdd.fiscalFunds = 0;
    this.genericAdd.noFiscalFunds = 0;
  },
  methods: {
    getcategoryName(data) {
      this.assetsCategoryName = data.categoryName;
      this.genericAdd.assetsCategoryName = data.categoryName;
      this.genericAdd.assetsCategory = data.code;
      this.getList();
      this.loading = false;
    },
    // 获取部门数据
    ...
    submit() {
      ...
      if (validResult.result) {
        ...
        R.Assets.genericAdd(genericModel.dispose(this.genericAdd)).then(resp => {
          if (resp.ok) {
            this.loading = false;
            this.$Message.success('保存成功');
            this.genericAdd = genericModel.parse({});
            this.genericAdd.assetsCategoryName = this.assetsCategoryName;
          } else {
            this.loading = false;

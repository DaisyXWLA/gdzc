        ...
        <Cell :width="24">
          <div id="print">
            <Form mode="threecolumn" :label-width="150">
              ...
              <FormItem label="建筑结构：">{{AssetsAdd.buildingStructureName}}</FormItem>
              <FormItem label="投入使用日期：">{{AssetsAdd.useTime}}</FormItem>
              <FormItem label="折旧年限（月）：">{{AssetsAdd.depreciationYear}}</FormItem>
              <FormItem label="竣工日期：">{{AssetsAdd.completionTime}}</FormItem>
              <FormItem label="权属性质：">{{AssetsAdd.ownershipNatureName}}</FormItem>
              <FormItem label="构建物计量：">{{AssetsAdd.constructionMeasure}}</FormItem>
              <FormItem label="坐落位置：">{{AssetsAdd.location}}</FormItem>
            </Form>
            <Form mode="threecolumn" :label-width="150">
...
export default {
  ...
  methods: {
    getAssetsId(data) {
      ...
      this.AssetsAdd.ownershipNatureCode = 'ownershipNature';
      this.AssetsAdd.buildingStructureCode = 'buildingStructure';
      this.AssetsAdd.isReportCode = 'isReport';
      R.Assets.houseFindOne(AssetsModel.dispose(this.AssetsAdd)).then(resp => {
        if (resp.ok) {
          this.loading = false;

<template>
  <div>
    <!-- 柱状统计图 -->
    <div id="expense-line-echarts" style="height:600px;"></div>
    <Button
      class="h-btn h-btn-s h-btn-blue export-btn"
      @click="exportExcel('expenseHistogram')"
    >导出Excel</Button>
    <div id="expenseHistogram">
...
export default {
  ...
  methods: {
        ...
        R.repairStatistics.expenseAssetsCategory(repairStatisticsModel.dispose(this.repairStatisticsList)).then(resp => {
          ...
          if (resp.ok) {
            ...
            for (let i = 0; i < this.datas.length; i++) {
              ...
              this.ydatas.push(this.datas[i].price);
            }
            this.histogram();
          }
        });
    ...
    histogram() {
      let expenseLineChart = echarts.init(document.getElementById('expense-line-echarts'));
      let option = {
        color: ['#8EC9EB'],
        // 鼠标悬浮提示
        tooltip: {
            ...
            itemStyle: {
              normal: {
                color: function (params) {
                  var colorList = ['#87C4E3', '#F9675A', 'gray', '#A5D98E', '#AD93F1'];
      ...
      expenseLineChart.setOption(option);
    },
    // 导出excel兼容主流浏览器
    getExplorer() {

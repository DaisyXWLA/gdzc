<template>
  <div>
    <Modal v-model="repairAdd" hasDivider :closeOnMask="closeOnMask" :middle="true">
      ...
      <div>
        ...
        >
          ...
          <FormItem label="报修人" prop="repair">
            <!-- <input type="text" v-model="repair" readonly /> -->
            <Select
...
export default {
  props: ['repairAdd', 'currentTime', 'repair', 'userOption'],
  components: {
    // selectAssets,
    uploadFile
  },
  ...
  mounted() {
    this.getData();
    this.assetsRepairList.repair = Utils.getLocal('userId');
  },
  methods: {
    // 选择报修人获取报修人所属资产
    getData() {
      this.assetsRepairList.userId = Utils.getLocal('userId');
      this.isLoading = true;
    ...
    submit() {
      ...
      if (validResult.result) {
        ...
        this.assetsRepairList.repairTime = this.currentTime;
        this.assetsRepairList.userId = Utils.getLocal('userId');
        if (this.assetsRepairList.ids.length <= 0) {
          this.$Message.info('请选择要报修的资产');
          return;
        }
        this.$Loading();
        R.assetsRepair.save(assetsRepairModel.dispose(this.assetsRepairList)).then(resp => {
          if (resp.ok) {
            setTimeout(() => {
              this.$parent.close();
              this.$Loading.close();
    ...
    //   R.assetsRepair.clickDelete(assetsRepairModel.dispose(this.assetsRepairList)).then(resp => {
    //     if (resp.ok) {
    //       setTimeout(() => {

  ...
  methods: {
    submit() {
      if (this.$refs['selectData'].select().length < 1) {
        ...
        this.subContractorRepairList.ids = this.$refs['selectData'].select();
        R.subContractorRepair.clickConfirm(subContractorRepairModel.dispose(this.subContractorRepairList)).then(resp => {
          if (resp.ok) {
            this.$emit('close');
            this.$Loading.close();

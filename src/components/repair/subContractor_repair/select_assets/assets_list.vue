      ...
      <Cell>
        <div class="h-panel">
          <div class="h-panel-bar" style="border:none;padding-bottom:0">
            <label>资产编号：</label>
            <input type="text" v-model="subContractorRepairList.newCode" style="width:24%" />&nbsp;&nbsp;
            <label>资产名称：</label>
            <input type="text" v-model="subContractorRepairList.assetsName" style="width:24%" />&nbsp;&nbsp;
        ...
        <div class="h-panel">
          <div class="h-panel-body assets-list">
            <Table ref="table" :datas="datas" :loading="isLoading" :height="457" checkbox>
              ...
              <TableItem title="使用部门" prop="useDepartment" align="left" :width="100"></TableItem>
              <TableItem title="管理部门" prop="manageDepartment" align="left" :width="100"></TableItem>
              ...
              <TableItem title="使用人" prop="useId" align="center" :width="80"></TableItem>
  ...
  methods: {
    ...
    searchUseCategory(data) {
      if (this.value == 1 && data != 1) {
        this.subContractorRepairList.useDepartment = '';
        this.subContractorRepairList.assetsCategory = data;
        this.getData();
      } else if (this.value == 2) {
        this.subContractorRepairList.assetsCategory = '';
        this.subContractorRepairList.useDepartment = data;
        this.getData();
    ...
    // 重置筛选条件
    reset() {
      this.subContractorRepairList.newCode = '';
      this.subContractorRepairList.assetsName = '';
    },
    // 获取所有数据列表
    getData() {
      this.isLoading = true;
      this.subContractorRepairList.page = this.pagination.page;
      this.subContractorRepairList.size = this.pagination.size;
      R.subContractorRepair.findAllAssets(subContractorRepairModel.dispose(this.subContractorRepairList)).then(resp => {
        // console.log(resp)
        if (resp.ok) {
    ...
    selectData() {
      if (this.$refs.table.getSelection().length < 1) {
        ...
        this.subContractorRepairList.ids = selectData.join(',');
      }
    },
    select() {
      this.selectIds = [];

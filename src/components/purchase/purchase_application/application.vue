        ...
        <div class="h-panel">
          <div class="h-panel-body">
            ...
            <Button
              ...
              icon="h-icon-trash"
              @click="remove"
              v-has="'purchaseApplicationdelete'"
            >删除</Button>
            ...
            >
              ...
              <TableItem title="申请编号" :width="200" align="left">
              ...
              <TableItem title="采购类型" :width="80" align="left">
                <template slot-scope="{ data }">
              ...
              <TableItem title="申请金额(元)" prop="price" :width="100" align="right"></TableItem>
              ...
              <TableItem title="操作" prop="operation" align="center" fixed="right" :width="150">
                <template slot-scope="{ data }">
                  <button
                    ...
                    @click="modify(data)"
                    v-has="'purchaseApplicationupdate'"
                  >
              ...
              <div slot="empty" class="wp-table-empty">暂时无数据</div>
            </Table>
            <p v-height="2"></p>
            <div>
              <Pagination
...
<script>
import planAdd from './application_add';
import planModify from './application_modify';
  ...
  methods: {
    ...
    keyupEnter() {
      document.onkeydown = e => {
        if (e.keyCode === 13 && e.target.baseURI.match(/purchaseApplication/)) {
          this.search();
        }
      };
    },
    // 详情
    view(datas, data) {
      this.$router.push({ path: '/applicationDetail/', query: { id: data.id, status: data.approvalEntity.staut } });
    },
    // 根据筛选条件查询
    search() {
    ...
    getData() {
      ...
      this.PurchaseList.page = this.pagination.page;
      this.PurchaseList.size = this.pagination.size;
      R.Purchase.PurchaseAList(PurchaseModel.dispose(this.PurchaseList)).then(resp => {
        if (resp.ok) {
          setTimeout(() => {
    ...
    modify(data) {
      if (data.approvalEntity.staut == 1) {
        this.$Message('申请审批中，不可编辑');
      } else if (data.approvalEntity.staut == 2) {
        this.$Message('申请已完成审批，不可编辑');
      } else {
    ...
    trigger(data) {
      if (data == 'modify') {
        ...
        this.$router.push({ path: '/applicationDetail/' + this.oneData.id });
      } else if (data == 'delete') {
        if (this.oneData.approvalEntity.staut == 1) {
          this.$Message('申请正在审批中，不可删除');
        } else {
          this.deleteModal = true;
        }
      }
    ...
    frontEndExportExcel() {
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, '采购申请');
      });
    },

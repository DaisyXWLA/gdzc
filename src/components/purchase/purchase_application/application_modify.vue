...
<template>
  <div>
    ...
    <header class="h-modal-header modal-title">编辑采购申请</header>
    <div>
      <div>
        <div style="padding:5px">
          ...
          >
            ...
            <FormItem label="采购类型" prop="type">
              <Select v-model="PurchaseAdd.type" :null-option="false" :datas="typeParam"></Select>
            </FormItem>
            ...
            <FormItem label="申请人" prop="creator">
              <!-- <input type="text" v-model="PurchaseAdd.creatorName" readonly /> -->
              <Select v-model="PurchaseAdd.creator" :datas="user" :filterable="true"></Select>
            </FormItem>
            <FormItem label="申请时间" prop="createTime">
              <!-- <input type="text" v-model="PurchaseAdd.createTime" readonly /> -->
        ...
        <Row>
          <Cell :width="24">
            <Table ref="table" :datas="datas" :height="159" @select="selectData" checkbox>
    ...
    <footer class="h-modal-footer">
      <button class="h-btn h-btn-primary" @click="save">更新</button>
...
export default {
  ...
  data() {
    return {
      ...
      validationRules: {
        required: ['title', 'type', 'time', 'price']
      },
      loading: false,
      deleteBtn: false,
      deleteModal: false
  ...
  methods: {
    // 获取申请人
    getUser() {
      R.pullDownList.user(PurchaseModel.dispose(this.PurchaseAdd)).then(resp => {
        if (resp.ok) {
          this.user = resp.body;
        }
      });
    ...
    savePurchaseAssets(data) {
      if (data.price + ''.length > 0 && data.amount + ''.length > 0 && data.specification.length > 0 && data.purchaseAssetsName.length > 0) {
        ...
        R.PurchaseAssets.UpdatePurchaseAssets(PurchaseAssetsModel.dispose(this.PurchaseAssetsAdd)).then(resp => {
          if (resp.ok) {
    ...
    getModifyData(id) {
      ...
      R.PurchaseAssets.PurchaseAssetsList(PurchaseAssetsModel.dispose(this.PurchaseAssetsAdd)).then(resp => {
        if (resp.ok) {
          resp.body.forEach(v => {
            v.purchaseAssetsNameClass = 0;
            v.specificationClass = 0;
            v.amountClass = 0;
            v.priceClass = 0;
          });
    ...
    save() {
      ...
      if (validResult.result) {
        ...
        R.Purchase.UpdatePurchase(PurchaseModel.dispose(this.PurchaseAdd)).then(resp => {
          if (resp.ok) {

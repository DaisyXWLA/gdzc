    ...
    <Modal v-model="importModal" v-width="800">
      ...
      <div v-width="800" style="padding:15px">
        ...
        <Table v-show="list" ref="table" :datas="datas" :loading="isLoading" :height="201">
          ...
          <TableItem title="用户姓名" prop="realName" align="left" :width="150"></TableItem>
          <TableItem title="所属部门" prop="departmentId" align="left" :width="150"></TableItem>
        </Table>
      </div>
      <div slot="footer">
  ...
  methods: {
    submit() {
      if (this.uploadIds.length == 0) {
        ...
        this.$Loading();
        this.userList.id = this.uploadIds.toString();
        this.userList.secondParameter = this.value;
        R.userManage.userImport(userModel.dispose(this.userList)).then(resp => {
          if (resp.ok) {
            this.$Loading.close();
            this.msg = resp.msg;

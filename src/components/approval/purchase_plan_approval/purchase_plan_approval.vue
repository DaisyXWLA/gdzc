        ...
        <div class="h-panel">
          <div class="h-panel-body">
            <Button
              ...
              icon="h-icon-plus"
              @click="approval"
              v-has="'purchasePlanApprovalapprove'"
            >审批</Button>
            <Button
              v-show="deleteBtn"
              color="green"
              icon="h-icon-edit"
              @click="allAgree"
              v-has="'purchasePlanApprovalagree'"
            >批量同意</Button>
            <Button
              v-show="deleteBtn"
              color="yellow"
              icon="h-icon-trash"
              @click="allRefuse"
              v-has="'purchasePlanApprovalrefuse'"
            >批量拒绝</Button>
            ...
            <Table ref="table" :datas="datas" :loading="isLoading" checkbox @select="selectBth">
              ...
              <TableItem title="计划编号" :width="120" align="left">
  ...
  methods: {
    ...
    keyupEnter() {
      document.onkeydown = e => {
        if (e.keyCode === 13 && e.target.baseURI.match(/purchasePlanApproval/)) {
          this.search();
        }
      };
    },
    ...
    getData() {
      ...
      this.PurchaseList.page = this.pagination.page;
      this.PurchaseList.size = this.pagination.size;
      R.Approval.findAllPlanApproved(PurchaseModel.dispose(this.PurchaseList)).then(resp => {
        if (resp.ok) {
    ...
    approval() {
      if (this.$refs.table.getSelection().length < 1) {
        ...
        if (this.$refs.table.getSelection().length >= 2) {
          ...
          this.$router.push({
            path: `/planApprovalDetail/${this.$refs.table.getSelection()[0].id}`
          });
        }
      }
    },

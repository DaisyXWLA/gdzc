<template>
  <div>
    <Modal v-model="retirementAdd" hasDivider :closeOnMask="closeOnMask" :middle="true">
      ...
      <div>
        ...
        >
          ...
          <FormItem label="报废编号" prop="code">
            <input type="text" v-model="ScrappedList.code" placeholder="编号自动生成" readonly disabled />
          </FormItem>
          <FormItem label="申请人" prop="applicant">
            <!-- <input type="text" v-model="applicant" readonly /> -->
...
export default {
  props: ['retirementAdd', 'currentTime', 'user'],
  components: {
    selectAssets,
    uploadFile
  },
  ...
  created() {},
  mounted() {
    this.ScrappedList.applicant = Utils.getLocal('userId');
    this.getData();
  },
  methods: {
    getData() {
    ...
    submit() {
      ...
      if (validResult.result) {
        ...
        this.ScrappedList.applicantTime = this.currentTime;
        this.ScrappedList.applicant = this.apply;
        this.$Loading();
        R.Scrapped.ScrappedAdd(ScrappedModel.dispose(this.ScrappedList)).then(resp => {
          if (resp.ok) {
            setTimeout(() => {
              this.$parent.close();
              this.$Loading.close();

<template>
  <div>
    <Modal v-model="retirementModify" hasDivider :closeOnMask="closeOnMask" :middle="true">
          ...
          <div>
            ...
            >
              <FormItem label="标题" prop="title" single>
                <input type="text" v-model="ScrappedList.title" v-wordlimit="100" />
              </FormItem>
              <FormItem label="报废日期" prop="scrappedTime">
                <DatePicker v-model="ScrappedList.scrappedTime"></DatePicker>
              </FormItem>
              <FormItem label="报废编号" prop="code">
                <input
                  type="text"
              ...
              <FormItem label="申请人" prop="applicant">
                <!-- <input type="text" v-model="ScrappedList.applicantName" readonly /> -->
                <Select v-model="ScrappedList.applicant" :datas="user" :filterable="true"></Select>
              </FormItem>
              <FormItem label="申请日期" prop="applicantTime">
                <DatePicker v-model="ScrappedList.applicantTime"></DatePicker>
              </FormItem>
              <FormItem label="报废原因" prop="reasonForScrapping" single>
                <input type="text" v-model="ScrappedList.reasonForScrapping" v-wordlimit="500" />
              </FormItem>
              <FormItem label="备注" prop="remarks" single>
                <input type="text" v-model="ScrappedList.remarks" v-wordlimit="2000" />
              </FormItem>
            </Form>
          </div>
          <Row>
...
export default {
  props: ['retirementModify', 'dataId', 'currentTime', 'user'],
  components: {
    selectAssets,
    uploadFile
  },
  ...
  methods: {
    ...
    updateAssets() {
      this.ScrappedList.page = this.pagination.page;
      this.ScrappedList.size = this.pagination.size;
      R.Scrapped.updateAssets(ScrappedModel.dispose(this.ScrappedList)).then(resp => {
        if (resp.ok) {
          // console.log(resp)
          this.datas = resp.body;
          this.pagination.page = resp.page;
    ...
    submit() {
      ...
      if (validResult.result) {
        this.ScrappedList.uploadId = this.uploadIds.toString();
        let selectAssetsIds = [];
        for (let i = 0; i < this.datas.length; i++) {
          selectAssetsIds.push(this.datas[i].id);
        }
        this.ScrappedList.ids = selectAssetsIds;
        // this.ScrappedList.applicant = this.user;
        this.$Loading();
        R.Scrapped.ScrappedUpdate(ScrappedModel.dispose(this.ScrappedList)).then(resp => {
          if (resp.ok) {
            setTimeout(() => {
              this.$parent.close();
              this.$Loading.close();
    ...
    modalConfirm() {
      ...
      R.Scrapped.ScrappedSelectAssetsDelete(ScrappedModel.dispose(this.ScrappedList)).then(resp => {
        if (resp.ok) {
          setTimeout(() => {
            this.deleteModal = false;
            this.$Message.success('刪除成功');
            this.getShowUpdateAssetsData();
          }, 200);
        }
      });
    },
    ...
    cancel() {
      R.Scrapped.ScrappedSelectAssetsCancel(ScrappedModel.dispose(this.ScrappedList)).then(resp => {
    ...
    changePage() {
      this.ScrappedList.id = this.dataId;
      this.getShowUpdateAssetsData();
    },
    // 控制删除按钮
    selectData(data) {

<template>
  <div>
    <Modal v-model="transferModify" hasDivider :closeOnMask="closeOnMask" :middle="true">
        ...
        <div>
          <div>
            ...
            >
              ...
              <FormItem label="接收人部门" prop="receiverDepartmentName">
                <input type="text" v-model="transferAssetsList.receiverDepartmentName" readonly />
              </FormItem>
...
export default {
  props: ['transferModify', 'dataId', 'currentTime', 'applicant', 'userOption', 'departmentOption'],
  components: {
    uploadFile
  },
  data() {
  ...
  methods: {
    getData(id) {
      ...
      R.transferAssets.transferAssetsFindOne(transferAssetsModel.dispose(this.transferAssetsList)).then(resp => {
        if (resp.ok) {
          ...
          this.isLoading = true;
          this.transferAssetsList.userId = resp.body.applicant;
    ...
    selectUser(data) {
      ...
      R.pullDownList.findDepartmentNameByUser(transferAssetsModel.dispose(this.transferAssetsList)).then(resp => {
        if (resp.ok) {
          this.transferAssetsList.receiverDepartmentName = resp.body.title;

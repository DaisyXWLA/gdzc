        ...
        <div class="h-panel">
          <div class="h-panel-bar" style="border:none;padding-bottom:0">
            <Row>
              <Cell :width="24">
                <label>资产编号：</label>
                <input type="text" v-model="transferAssetsList.newCode" style="width:24%" />&nbsp;&nbsp;
                <label>资产名称：</label>
                <input type="text" v-model="transferAssetsList.assetsName" style="width:24%" />&nbsp;&nbsp;
  ...
  methods: {
    ...
    searchUseCategory(data) {
      if (this.value == 1 && data != 1) {
        this.transferAssetsList.useDepartment = '';
        this.transferAssetsList.assetsCategory = data;
        this.getData();
      } else if (this.value == 2) {
        this.transferAssetsList.assetsCategory = '';
        this.transferAssetsList.useDepartment = data;
        this.getData();
    ...
    // 重置筛选条件
    reset() {
      this.transferAssetsList.newCode = '';
      this.transferAssetsList.assetsName = '';
    },
    // 获取所有数据列表
    getData() {
      this.transferAssetsList.page = this.pagination.page;
      this.transferAssetsList.size = this.pagination.size;
      R.transferAssets.findAllAssets(transferAssetsModel.dispose(this.transferAssetsList)).then(resp => {
        // console.log(resp)
        if (resp.ok) {
    ...
    selectData() {
      if (this.$refs.table.getSelection().length < 1) {
        ...
        this.transferAssetsList.ids = selectData.join(',');
      }
    },
    select() {
      this.selectIds = [];

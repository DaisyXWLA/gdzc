<template>
  <div>
    <Modal v-model="useAdd" hasDivider :closeOnMask="closeOnMask" :middle="true">
      ...
      <div>
        ...
        >
          ...
          <FormItem label="领用部门" prop="useDepartment">
            <input type="text" v-model="useDepartment" readonly />
          </FormItem>
          <FormItem label="预计归还时间" prop="planTime">
            <DatePicker v-model="useReturnList.planTime"></DatePicker>
          </FormItem>
          <FormItem label="领用日期" prop="useTime">
            <DatePicker v-model="currentTime" placeholder="领用日期"></DatePicker>
          </FormItem>
          <FormItem label="领用编号" prop="code">
            <input type="text" v-model="useReturnList.code" placeholder="编号自动生成" readonly disabled />
          </FormItem>
...
export default {
  props: ['useAdd', 'currentTime', 'departmentOption', 'userOption'],
  components: {
    selectAssets,
    uploadFile
  },
  ...
  methods: {
    getData() {
      ...
      R.useReturn.showSelectAssets(useReturnModel.dispose(this.useReturnList)).then(resp => {
        if (resp.ok) {
    ...
    selectUser(data) {
      ...
      R.pullDownList.findDepartmentNameByUser(useReturnModel.dispose(this.useReturnList)).then(resp => {
        if (resp.ok) {
          this.useDepartment = resp.body.title;
    ...
    submit() {
      ...
      if (validResult.result) {
        ...
        this.useReturnList.useTime = this.currentTime;
        this.useReturnList.useDepartment = this.useDepartmentId;
        this.$Loading();
        R.useReturn.useReturnAdd(useReturnModel.dispose(this.useReturnList)).then(resp => {
          if (resp.ok) {
            setTimeout(() => {
              this.$parent.close();
              this.$Loading.close();
